...
`
	...
	for _, source := range []struct {
		...
		for _, name := range source.functions {
			excerpt := regexp.MustCompile(`(?ms)^(?:void|std::string|string|CFURLRef|SecStaticCodeRef) (?:SingleDiskRep::)?` + name + `\(.*?^}`).Find(data)
			if len(excerpt) == 0 {
	...
	record := map[string]any{
		...
		"scope":   "Six complete verbatim function bodies: CLI cleanPath/staticCodePath/note plus SingleDiskRep canonical path, executable path and identifier. Real SDK declarations supply realpath, PATH_MAX, stdio/variadic functions, CoreFoundation and public Security APIs/constants. Private error, Architecture, CFRef, CFTempURL, dictionary helper and SingleDiskRep interfaces are declaration-only shims; verbose is an extern declaration. AST records realpath before static-code creation, common path storage for display and identifier, and verbosity-gated stderr notices with a trailing newline. The current CLI signing call site is not published by the pinned source: replacement-notice timing, count, path spelling and conditions are established separately by native acceptance, not inferred from this helper. This driver does not execute Apple source or compile all Security. Production uses pure Go path resolution and go-apfs-v2 metadata writes, with no Apple runtime or SDK dependency.",
	...
	fmt.Println("Wrote spec/apple-paths.json: six path/notice functions on two targets")
}

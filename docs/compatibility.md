...
| Formats | Embedded Mach-O signatures; bounded APPL bundles with XML/binary metadata and resource envelopes; single-segment UDIF DMGs via go-apfs-v2 | Wider layouts/nested code, symlink/xattr policy, encrypted/segmented images, streaming large images, detached and generic files |

...
| Entitlements | XML/DER encoding and executable flags; bounded DER-first compact XML/typed text extraction, append lifecycle, slot binding and architecture selection | Legacy/alternate DER, full type/error/CMS-policy parity, constraints and hybrid signing interactions |
| Requirements | Identifier, CDHash, certificate index/root hash, subject CN/O/OU, extension existence, generic Apple anchor, boolean expressions; bounded named-set extraction and implicit designated requirements with native file lifecycle | Remaining predicates, Info.plist predicates, complete native grammar/set compilation, alternate/external slots and diagnostic output |
...
| Signature file lists | Ordered outer-representation paths, append/stdout, native framework spelling, output-open failure ordering and signed DMG dry-run reports | External CLI layouts, native crash differences, output/input aliases, special streams, complete errno/option interactions |
| Certificate extraction | Supported CMS chains as leaf-first DER, default/empty/custom prefixes, architecture selection, native overwrite/partial-write and continuation behavior; physical bundle-parent display | Wider CMS/chains, host augmentation, signature slots, broader extraction interactions and localized dates |
